...
def check_audio_captcha():
    if ans.get() == get_audio():
        tkinter.messagebox.showinfo("OK VERIFIED!","Captcha Code Matched.")
        ans.set("")
...
root = Tk()
root.title("CAPTCHA BY TURBO GROUPS")

root.geometry("842x498")

root.configure(background = '#000fff000')
...
Title_Label = Label(Tops,font=('Comic Sans MS',16,'bold'),text = "             TURBO GROUPS Presents to you :THE CAPTCHA Generation\t\t",bg= 'black',fg='yellow',justify ="center")
Title_Label.grid(row=0,column=0)
MainFrame = Frame(root,bg = 'cyan',pady = 2, width =1350, height = 100, relief = RIDGE)
...
Label_1 =Label(RightFrame, font=('lato black', 33,'bold'), text=" Audio Captcha ",padx=2,pady=2, bg="red",fg ="white")
...
Label_9 =Button(RightFrame, font=('arial', 19,'bold'), text="Play CAPTCHA",padx=2,pady=2, bg="blue",fg = "white",command=generate_audio_captcha)
...
Label_4 =Button(RightFrame, font=('arial', 15,'bold'), text="Regenerate CAPTCHA",padx=2,pady=2, bg="white",fg = "red",command=regenerate_audio_captcha)

...
def iExit():
    iExit=tkmsg.askyesno("Exit Restaurant System","Confirm if you want to exit")
    if iExit > 0:
        root.destroy()
        import newfile
        return
...
def Reset():
    ...
    txtChickenBurger.configure(state=DISABLED)
...
def CostofItem():
    ...
    Item12=float(E_ChickenBurger.get())
    ...
    PriceofFood =(Item9 * 150) + (Item10 * 115) + (Item11 * 128) + (Item12 * 290) + (Item13 * 75) + (Item14 * 110) + (Item15 * 130) + (Item16 * 99)



    DrinksPrice = "Rs",str(float(PriceofDrinks))
    FoodPrice =  "Rs",str(float(PriceofFood))
    CostofFood.set(FoodPrice)
    CostofDrinks.set(DrinksPrice)
    SC = "Rs",str(float(10))
    ServiceCharge.set(SC)

    SubTotalofITEMS = "Rs",str(float(PriceofDrinks + PriceofFood + 10))
    SubTotal.set(SubTotalofITEMS)

    Tax = "Rs",str(float((PriceofDrinks + PriceofFood + 10) * 0.15))
    PaidTax.set(Tax)

    TT=((PriceofDrinks + PriceofFood + 10) * 0.15)
    TC="Rs",str(float(PriceofDrinks + PriceofFood + 10 + TT))
    TotalCost.set(TC)
...
def chk_ChickenBurger():
    if(var12.get() == 1):
        txtChickenBurger.configure(state = NORMAL)
        txtChickenBurger.delete('0',END)
...
def Receipt():
    ...
    txtReceipt.insert(END,'ChickenBurger:\t\t\t\t\t'+ E_ChickenBurger.get()+'\n')
...
ChickenBurger = Checkbutton(Food_F,text="Rice Plate ",variable=var12,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='orange',command=chk_ChickenBurger).grid(row=3,sticky=W)
...
txtChickenBurger=Entry(Food_F,font=('arial',16,'bold'),bd=8,width=6,justify=LEFT,state=DISABLED,
                        textvariable=E_ChickenBurger)
txtChickenBurger.grid(row=3,column=1)
